...
class BoardingMapPicker extends StatefulWidget {
  final LatLng initialLocation; // Centro da cidade de origem
  final String? initialName; // Se estiver editando

  const BoardingMapPicker({
    super.key,
  ...
  @override
  State<BoardingMapPicker> createState() => _BoardingMapPickerState();
}

class _BoardingMapPickerState extends State<BoardingMapPicker> {
  ...
  void _onSearchChanged(String query) {
    ...
    _debouncer.run(() async {
      try {
        final bounds = _calculateBounds(widget.initialLocation);
        final result = await _places.findAutocompletePredictions(
          query,
          countries: ['BR'],
          locationBias: bounds,
          // Aqui permitimos ESTABELECIMENTOS também, pois é ponto exato
        );
        if (!mounted) return;
        setState(() {
  ...
  places.LatLngBounds _calculateBounds(LatLng center) {
    const double offset = 0.2; // Aprox 20km (1 grau ~= 111km)
    ...
    return Scaffold(
      body: Stack(
        children: [
          ...
          // 2. O PIN CENTRAL (Fixo)
          const Center(
            child: Padding(
              padding: EdgeInsets.only(
                bottom: 30,
              ), // Ajuste visual para a ponta do pin
              child: Icon(Icons.location_on, size: 45, color: Colors.black),
            ),
          ),

          // 3. BARRA DE BUSCA FLUTUANTE
          Positioned(
            top: 50,
            left: 16,
            right: 16,
                      ...
                      Expanded(
                        child: TextField(
                          ...
                          decoration: const InputDecoration(
                            hintText: 'Buscar local (ex: Shopping, Posto...)',
          ...
          Positioned(
            ...
            child: ElevatedButton(
              onPressed: () {
                // Se não buscou nome, usa "Local Selecionado no Mapa"
